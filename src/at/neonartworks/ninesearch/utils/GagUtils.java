...
public class GagUtils {
	...
	public static List<String> filter(String content) {
		...
		*/
		content = content.substring(content.indexOf(getSearchFilter()), content.indexOf(getSearchFilter2()));
		...
		for (String entrie : rawList) {
			...
			if (!entrie.equals("") || (!entrie.equals(" "))) {
				...
				if ((!id.equals("")))
					...
					postList.add(new GagPost(id, thumbnail, image, title, link, Integer.valueOf(comments), Integer.valueOf(votes)));
